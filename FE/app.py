...
from BE.services.maps import js_client_id  # noqa: E402
from BE.utils.config import clear_settings_cache, get_settings  # noqa: E402
...
def main() -> None:
    ...
    if not form.submitted and not session.get_result():
        st.info(
            ...
            "PRD 시나리오를 바로 실행하세요. (.env 키가 있으면 실데이터 사용)"
        )
        render_demo_panel()
        st.divider()
        _render_empty_guide()
    ...
    if result:
        ...
        client_id = js_client_id() or (
            settings.naver_map_client_id or settings.naver_openapi_client_id or None
        )
        # route 에 실려 온 client id 우선
        route = result.get("route") or {}
        client_id = route.get("naver_client_id") or client_id

        render_live_badge(result, maps_js=bool(client_id))
        render_course_result(result, naver_client_id=client_id)
...
def _render_pipeline_meta(result: dict) -> None:
    ...
    if result.get("saved"):
        st.caption(
            f"저장됨 · DB={result.get('db_kind')} · course_id={result.get('course_id')}"
        )
    elif result.get("course_id") is None:
        st.caption("저장되지 않았습니다. (결과만 표시)")
...
2. 단계: TourAPI → CLOVA → Maps → 저장
3. 실데이터 배지 + 장소 카드 + NAVER Maps 확인
"""
    )
